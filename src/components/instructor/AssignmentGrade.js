...
const AssignmentGrade = (props) => {
    ...
    const fetchGrades = async (id) => {
        try {
            const response = await fetch(`${SERVER_URL2}/assignments/${id}/grades`);
            if (response.ok) {
    ...
    const onSave = async () => {
        try { 
            const response = await fetch (`${SERVER_URL2}/grades`,
                {
                method: 'PUT',
                headers: {
        ...
        <>
            ...
            <Dialog open={open} >
                    ...
                    <table className="Center" > 
                        ...
                        <tbody>
                            {grades.map((g) => (
                                    ...
                                    <td><input id ="score-input" type="text"  name="score" value={g.score}  onChange={onChange} /></td>
                                    </tr>
                                ))}
                        </tbody>
                    </table>
                ...
                <DialogActions>
                    ...
                    <Button id="upload-scores" color="primary" onClick={onSave}>Save</Button>
                </DialogActions>
            </Dialog> 
        </>          
    );
